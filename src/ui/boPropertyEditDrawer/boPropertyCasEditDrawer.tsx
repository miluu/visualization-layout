...
interface IUiBoPropertyCasEditDrawerProps {
  onSubmit?(data: any): void;
}

interface IUiBoPropertyCasEditDrawerState {
  boPropertyCas: IBoPropertyCas;
  boProperty: IBoProperty;
  type: string;
  visible: boolean;
}

export class UiBoPropertyCasEditDrawer extends React.PureComponent<IUiBoPropertyCasEditDrawerProps, IUiBoPropertyCasEditDrawerState> {
  state: IUiBoPropertyCasEditDrawerState = {
    boPropertyCas: null,
    boProperty: null,
    type: 'edit',
    visible: false,
  ...
  open = ({ boPropertyCas, type, boProperty }: { boPropertyCas: IBoPropertyCas, boProperty: IBoProperty, type: string }) => {
    this.setState({
      visible: true,
      type,
      boPropertyCas,
  ...
  private isDataModified() {
    ...
    const { boPropertyCas } = this.state;
    return isFormDataModified(boPropertyCas, formValues);
  ...
  private save = () => {
    this.getForm().validateFields(null, { force: true }, (err) => {
      ...
      } else if (boPropertyCas.rowStatus === ROW_STATUS.NOT_MODIFIED) {
  ...
  private renderForm = () => {
    return (
      <div className="editor-drawer-form">
        <BoPropertyCasEditForm boProperty={this.state.boProperty} ref={this.formRef} />
      </div>
    );
  }
...
interface IBoPropertyCasEditFormProps {
...
@(Form.create({ name: 'BoPropertyCasEditForm' }) as any)
class BoPropertyCasEditForm extends React.PureComponent<IBoPropertyCasEditFormProps> {
  render() {
  ...
  private propertyNameQueryMehtod = async (options: IQueryOptions): Promise<IQueryResult> => {
    const { boProperty } = this.props;
    return queryPropertyNameMethod(options, boProperty?.ipfCcmBoId);
  }

  private subPropertyNameQueryMehtod = async (options: IQueryOptions): Promise<IQueryResult> => {
    const { boProperty } = this.props;
    return querySubPropertyNameMethod(options, boProperty?.subBoName);
  }

  private linkPropertyNameQueryMehtod = async (options: IQueryOptions): Promise<IQueryResult> => {
    const { boProperty } = this.props;
    return querySubPropertyNameMethod(options, boProperty?.linkBoName);
  }
}
